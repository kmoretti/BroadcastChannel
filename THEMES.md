...
BroadcastChannel always loads its complete **Base** theme. A theme selection is either Base alone or Base plus one CSS override loaded through `HEADER_INJECT`. Overrides change presentation only: BroadcastChannel still owns the routes, complete Memos feed, reactions, comments, media, search, RSS, and other product behavior.
...
HN News is a compact, utilitarian full-post feed with a warm light canvas, orange accent, dense metadata, and a story-title hierarchy above each available post title; it does not collapse posts into summaries. ZAE uses a light, compact document sheet with a reduced identity header, technical toolbar, restrained rules, and monospace accents. These two themes deliberately fix `color-scheme: light`; they do not switch with the system preference.

Each is an independent visual interpretation for BroadcastChannel, not a source adaptation or an official theme. No source code, CSS, fonts, or assets from Hacker News or Zed are bundled. See [NOTICE.md](./NOTICE.md) for provenance and non-affiliation details.
